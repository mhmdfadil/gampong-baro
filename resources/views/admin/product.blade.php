...
<style>
  ...
  .dataTables_wrapper .dataTables_filter input {
    ...
    @media screen and (max-width: 767px) {
        .table tbody td[data-label="Gambar"] img {
            width: 40px;
            height: 40px;
        }
        ...
        <div class="col-lg-12">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap">
                    <div class="mb-3">
                        <h4 class="m-0 font-weight-bold text-primary">
                            <i class=""></i> Manajemen Belanja
                        </h4>
                    </div>
                    <div class="d-flex align-items-center">
                        <span class="badge bg-primary rounded-pill px-3 py-2">
                            <i class="fas fa-list mr-1"></i> Daftar Produk
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="card setting-card mb-4">
                <div class="card-header setting-header text-white d-flex justify-content-between align-items-center flex-wrap">
                    <p class="mb-2 mb-md-0">Kelola semua produk yang tersedia</p>
                    <div>
                        ...
                        <table id="productTable" class="table table-hover" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td data-label="Lokasi">{{ $product->location }}</td>
                                    <td data-label="Harga">
                                    ...
                                    <td data-label="Rating">
                                        @if($product->rating)
                                            <span class="badge bg-success">{{ number_format($product->rating, 1) }}</span>
                                        @else
                                    ...
                                    <td data-label="Label">
                                        @if($product->badge)
                                            <span class="badge rounded-pill {{ $product->badge_class ?? 'bg-primary' }}">
                                                {{ $product->badge }}
                                            </span>
                                        @else
                                    ...
                                    <td data-label="Aksi">
                                        <div class="d-flex gap-2 justify-content-center flex-wrap">
                                            ...
                                            <button onclick="window.location.href='{{ route('products.edit', $product->id) }}'" 
    ...
    $(document).ready(function() {
        var table = $('#productTable').DataTable({
            ...
            buttons: [
                {
                    ...
                    exportOptions: {
                        columns: [0, 1, 2, 3, 4, 5, 6, 7], // Exclude action column (index 8)
                        format: {
                            body: function ( data, row, column, node ) {
                                if (column === 1) {
                                    return $(node).find('img').data('filename') || '';
                                }
                                return data;
                            }
                        }
            ...
            initComplete: function() {
                this.api().buttons().container().appendTo('#productTable_wrapper .col-md-6:eq(0)');
            },
            columnDefs: [
                { responsivePriority: 1, targets: 0 }, // No
                { responsivePriority: 2, targets: -1 }, // Aksi
                { responsivePriority: 3, targets: 2 }, // Nama
                { responsivePriority: 4, targets: 3 }, // Lokasi
                { responsivePriority: 5, targets: 4 }, // Harga
                { 
                ...
                { responsivePriority: 7, targets: 5 }, // Rating
                { responsivePriority: 8, targets: 6 }, // Kategori
                { responsivePriority: 9, targets: 7 }, // Label
                { 
